...
本项目是一个基于YOLO的目标检测系统，专为树莓派4B平台设计，用于实时检测视频流中的摔倒事件。系统能够自动识别监控画面中的摔倒行为，并触发相应警报机制，适用于老年人看护、安全监控等场景。

作者: zhangpeng

## 简介

本项目基于树莓派4B开发，旨在提供一个轻量级但高效的摔倒检测解决方案。通过在边缘设备上部署深度学习模型，系统能够在保护用户隐私的同时实现实时摔倒检测。

主要特性：
- 基于YOLOv8的目标检测算法
- 专为树莓派4B优化的轻量级模型
- 实时视频流处理能力
- 多种警报机制（日志记录、语音播报、API通知等）
- 边缘计算，保护用户隐私
